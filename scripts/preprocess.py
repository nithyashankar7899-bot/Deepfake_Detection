...
# OpenCV ships this cascade file inside the package itself — no extra download,
# no compiling. This replaces the old face_recognition/dlib dependency, which
# has no pre-built wheel for newer Python versions on Windows.
...
def crop_face(frame, margin=0.3):
    ...
    x, y, box_w, box_h = max(faces, key=lambda b: b[2] * b[3])

    h, w = frame.shape[:2]
    pad_h, pad_w = int(box_h * margin), int(box_w * margin)

    top = max(0, y - pad_h)
    bottom = min(h, y + box_h + pad_h)
    left = max(0, x - pad_w)
    right = min(w, x + box_w + pad_w)
